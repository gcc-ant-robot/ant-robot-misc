...
%test 8 different time facillitation constants
for FT_IND=0:8
  switch FT_IND
    case 0
      wb = 0.04
...
%%%%%%%%%%% input paths %%%%%%%%%%%%
codeDirectory=['C:\Users\stahllr1\OneDrive - Grove City College\Documents\GCC\5_Fall 2018\COMP 370 - Research\IIT_Model'];
video_path=[codeDirectory '\images\'];
resultsDir = [codeDirectory '\results'];

%Manually set values commented out below
degrees_in_image =50;
X_C = 136; %Ground truth x center
Y_C = 68; %Ground truth y center  
...
dataGreenChannel=[];
for IN=1:nFrames
    Frame=imread([video_path img_files(IN).name]);
...
%input(Prompt1); % Facilitation time constant
...
%wb =0.05;%input(Prompt); % Facilitation time constant
%if isempty(wb)
%    wb=0.05;
%end
...
save(strcat('TargetLocationRow', num2str(round(wb*1000))), 'TargetLocationRow')
save(strcat('TargetLocationCol', num2str(round(wb*1000))), 'TargetLocationCol')
save(strcat('framePTime', num2str(round(wb*1000))), 'framePTime')
save(strcat('blurPTime', num2str(round(wb*1000))), 'blurPTime')
clear functions
